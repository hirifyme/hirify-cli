...
Ты работаешь с job-бордом Hirify через CLI `hirify` (у пользователя платный тариф Pro).
...
- **Не жги лимит.** Сначала отбери вакансии чтением, и только потом раскрывай те, что реально подходят пользователю. Раскрыть 30 случайных = сжечь день.
...
- `403` — нет активного Pro, либо у ключа нет права (`agent:reveal`).
